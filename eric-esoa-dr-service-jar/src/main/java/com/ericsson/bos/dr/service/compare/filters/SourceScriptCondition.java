...
/**
 * Groovy script condition executed in the context of the source object.
 */
@Component
public class SourceScriptCondition extends ScriptCondition {
    ...
    public boolean test(ApplicationConfigurationFilterDto filterDef, FilterContext filterCtx) {
        ...
        scriptArgs.put("source", filterCtx.getSourceProperties());
        scriptArgs.put("targets", filterCtx.getTargetsProperties());
        return test(filterDef,filterCtx, scriptArgs);
    }

    @Override
    public boolean supports(ApplicationConfigurationFilterDto filterDef) {
        return filterDef.getCondition() != null ? FilterConditionDto.NameEnum.SOURCESCRIPT.equals(filterDef.getCondition().getName()) : false;
    }
