...
class CreateJobNegativeBase extends Specification {
    ...
    @Before
    void setup() {
        final ExecuteJobDto executeJobDto = new ExecuteJobDto().name("jobName").featurePackId("1").applicationId("404")
                .applicationJobName("applicationJobName").description("description").inputs([:])
        doThrow(new DRServiceException(ErrorCode.FP_NOT_FOUND, "1")).when(discoveryService).startDiscovery(executeJobDto as ExecuteJobDto)
