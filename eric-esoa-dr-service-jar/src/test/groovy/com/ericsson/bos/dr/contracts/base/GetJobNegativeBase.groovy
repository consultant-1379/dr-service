...
class GetJobNegativeBase extends Specification {
    ...
    @Before
    void setup() {
        doThrow(new DRServiceException(ErrorCode.GENERAL_ERROR, "Error")).when(jobService).getJobById("500")
        doThrow(new DRServiceException(ErrorCode.JOB_NOT_FOUND, "404")).when(jobService).getJobById("404")
